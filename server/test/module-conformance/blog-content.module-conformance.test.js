...
test("blog content creates deterministic revisions and enforces lifecycle role checks", async () => {
  ...
  try {
    ...
    const pagesAfterCreate = await injectJson(
      server,
      "GET",
      `${buildItemsRoute("blog-pages")}?limit=200`
    );
    expect(pagesAfterCreate.statusCode).toBe(200);
    expect(
      pagesAfterCreate.body.items.some((page) => page.sourcePostId === createPost.body.item.id)
    ).toBe(false);
