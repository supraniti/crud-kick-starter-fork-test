...
export function DistributionQueue({ posts, selectedPostId, authorMap, readinessMap, onSelectPost }) {
  return (
    <Paper variant="outlined" sx={{ p: 2 }}>
      <Stack spacing={2}>
        <Typography variant="h6">Distribution Queue</Typography>
        {posts.length === 0 ? (
          <Alert severity="info">No posts match the current distribution filters.</Alert>
        ) : null}
        <Stack spacing={1.5}>
          {posts.map((post) => {
              ...
              >
                <Stack spacing={1}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Scheduled {formatTimestamp(post.scheduledOn)}
                  </Typography>
                </Stack>
              </Paper>
            );
...
export function ReadinessPanel({ workspace }) {
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2 }}>
      <Stack spacing={2}>
        <Typography variant="h6">Readiness Detail</Typography>
        {!post ? (
          <Alert severity="info">Select a post from the distribution queue.</Alert>
        ) : (
          <>
            <Stack direction="row" spacing={1} flexWrap="wrap">
              ...
              <Chip
                size="small"
                label={`Permalink /blog/${post.slug ?? post.id}`}
                variant="outlined"
              />
              <Chip
                size="small"
