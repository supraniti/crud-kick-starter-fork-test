...
const CONNECTIONS_COLLECTION_ID = "remote-connection-profiles";
...
export async function readStoredRemoteConnectionProfileById(connectionId) {
  if (typeof connectionId !== "string" || connectionId.trim().length === 0) {
    return null;
  }

  const snapshot = await readRemoteOpsStateSnapshot();
  const items = Array.isArray(snapshot?.[CONNECTIONS_COLLECTION_ID])
    ? snapshot[CONNECTIONS_COLLECTION_ID]
    : [];
  return items.find((item) => item?.id === connectionId) ?? null;
}
