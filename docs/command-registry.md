...
- What the review launcher does:
  ...
  - serves `frontend/dist` through the repo-owned static server on `localhost:3000`
  - verifies both endpoints before returning success
- Open the app at:
  - `http://localhost:3000`
...
- For manual review on this machine, prefer the static review launcher over Vite. It avoids the recurring local `spawn EPERM` failure path.
